  ...
  class Main
    ...
    def load_meta_information
      @meta_information.keys.each do |index|
        ...
        theme    = @survey.themes.where(name: q[:theme]).first_or_create
    ...
    def load_responses
      @responses.each do |r|
        response = @survey.responses.create(email: r[:email], employee_id: r[:employee_id], submitted_at: r[:submitted_at])
        answers = r[:answers]
